...
def get_rollingDates(target_folder):
    dateslist = []
    for f in os.scandir(target_folder): #scan through all the file name in the path
        if f.is_dir():
            y = f.name.split('-')[0] # get the year in the file name
            if (y < '2022')|(y > '2024'):
                # print(f.name)
                continue
            else:
...
def get_ETF_Data(date:str, inst_name:str = '510300.XSHG'):
    ...
    ETF_oneday = inst_obj.kline_day # get ETF_Data through kline data
    return ETF_oneday 
...
def get_parallel_ETF_Data(trading_date_list:list):
    ...
    ) # use multi-processing to get the ETF data of the instrument '510300.XSHG'
...
def get_strikes(ETF_daily:pd.DataFrame, start_date:str = '2022-02-11', end_date:str = '2024-01-10'): 
    ...
    # 获取期权合约的虚一档行权价格 (虚一档行权价格：最接近市场价格的虚值期权(OUT-OF-THE-MONEY))
    for i in ETF_daily.index:
        ...
        if (ETF_daily.loc[i,'ETF_open']*100%10) < 5: # 平值(ATM)
        ...
        ETF_daily.loc[i,'otm1_call'] = round(ETF_daily.loc[i,'atm_call']+gap,2) # 计算虚值期权(OTM)保留两位小数
    ...
    ETF_daily.to_csv('22-24_ETF_daily.csv')
    return ETF_daily




def get_option_daily(date, ETF_daily, signal_ttm:int = 57, open_minTime:int = 4, close_minTime:int = 229): # signal_ttm(signal time-to-maturity距离合约的日子) 
    # open_minTime 开盘时间 close_minTime 收盘时间
    ...
    for column in ['atm_call','otm1_call','otm2_call','otm3_call','otm4_call','itm1_call','itm2_call','itm3_call','itm4_call','itm5_call', 'itm6_call']: 
        df = option_traded[(pd.to_numeric(option_traded['symbol'].str[-4:]) == 1000*ETF_daily.loc[date, column])] #提取期权代码后四位进行对比返回Boolean df
        #保留返回True的行
        df = df.sort_values('time_to_maturity').copy() #对df根据‘time-to-maturity'(离到期日的距离)进行排列
        df.reset_index(drop=True, inplace=True) #重置后的df的index不再具有意义，重新设置新的index

        for i in df.index:
            if df.loc[i,'time_to_maturity'] >= signal_ttm: #寻找第一个'time to maturity' >= "signal ttm" 的期权合约，提取该合约的信息并退出循环
        ...
        target_greeks_dict = greeks_df.iloc[open_minTime].to_dict() # 一致 提取greeks_df第 open_minTime 行并转成字典
        target_greeks_dict['strike'] = round(int(target_greeks_dict['symbol'][-4:])*0.001,1) # 修改字典里面的values
        ...
        option_dict_list.append(target_greeks_dict) # 每次循环将target_greeks_dict 字典添加到option_dict_list里面
   
    option_daily_df = pd.DataFrame(option_dict_list)  # 返回一个dataframe 
    option_daily_df['type'] = [option_type]*option_daily_df.shape[0] # 在这个dataframe里添加期权类型的列
...
def get_parallel_option_daily(ETF_daily, trading_date_list:list, signal_ttm:int = 45, open_minTime:int = 4, close_minTime:int = 229):
    ...
    option_daily_df.rename({'trading_date':'time','contract_id':'inst', 'IV':'open_iv', 'delta':'open_delta','gamma':'open_gamma','vega':'open_vega','theta':'open_theta'}, axis = 1, inplace = True)# axis = 1 对列进行重命名
    ...
    option_daily_df = pd.merge(ETF_daily, option_daily_df,on = 'time') # 合并ETF_daily和option_daily_df 根据time列
    
    print('finish strike calculation!!', option_daily_df)
    option_daily_df.to_csv('45_option_daily.csv')
    return option_daily_df
...
def get_timing_signal(option_iv, otm_level = 'otm1_call', timing_path = '/home/zhangyan/option_skew/timing_signal.csv', start_date='2022-01-28', end_date = '2024-01-10'):
    timing_df = pd.read_csv(timing_path, index_col = 0) # 读取time_path 文件
    timing_df['date'] = pd.to_datetime(timing_df['date']) # 将date string转成datatime 
    factor_df = timing_df[(timing_df['date']>=start_date)&(timing_df['date']<=end_date)] # 从time_df里面筛选指定时间范围内的信息
    factor_df.reset_index(drop = True, inplace = True) # 筛选完后重置index
    
    # 计算和更新时机的信号
    for i in range(1, factor_df.shape[0]): 
        date = str(factor_df.loc[i, 'date'])[:10] # 提取今日和前一日
        last_date = str(factor_df.loc[i-1, 'date'])[:10]
        
        option_oneday =  option_iv.loc[option_iv['time'] == date] # 根据date 和 last date 从 opiton_iv 提取当天和前一天的期权数据
        ...
        if (factor_df.loc[i, 'isHedge_signal'] != 0)&(factor_df.loc[i-1, 'isHedge_signal'] == 0):  # 如果当前有对冲信号且前一天没有 开仓
            factor_df.loc[i, 'final_signal'] = factor_df.loc[i, 'isHedge_signal']   # -1
            factor_df.loc[i, 'direction'] = factor_df.loc[i, 'isHedge_signal']    # -1
        elif (factor_df.loc[i, 'isHedge_signal'] == 0)&(factor_df.loc[i-1, 'isHedge_signal']!= 0):  # 如果当前没有对冲信号且前一天有 平仓
    ...
    factor_df.to_csv(f'22-24_{otm_level[:4]}_atm_timing_test.csv')
    return factor_df
...
def get_open_infos(far_atm_delta, far_max_otm_delta, direction, ETF_open, ETF_pre_close, new_atm_open, new_max_otm_open, contract_multiplier, new_atm_delta, new_max_otm_delta, init_cash = 1000000, risk_expo = 0.2):
    ...
    while abs(far_cash_delta) > init_cash*risk_expo:
    ...
    # margin  保证金
    if direction == -1:
        max_otm_margin = otm_num*ETF_pre_close*contract_multiplier*0.12                     # short max otm 卖出最大虚值期权的保证金
        atm_margin = atm_num*new_atm_open*contract_multiplier                               # long atm 买入平值期权的保证金
    elif direction == 1:
        max_otm_margin = otm_num*new_max_otm_open*contract_multiplier                       # long max otm 买入最大虚值期权的保证金
        atm_margin = atm_num*ETF_pre_close*contract_multiplier*0.12                         # short atm  卖出平值期权的保证金  
    total_margin = max_otm_margin + atm_margin # 无论买卖期权都交保证金, 不区分符号 总保证金
...
def get_margin_back(date, ETF_pre_close, last_str, option_current_date, last_option_current_date, contract_multiplier, last_contract_multiplier, option_commission:int = 3, open_minTime:int = 4):
    if last_str is not None: # 提取上一日头寸的信息
        ...
        if (last_max_otm_inst != max_otm_inst)|(last_atm_inst != atm_inst):      
            ...
            if dirc == -1: # dirc 为持仓方向，dirc=1(多仓(long position)) 即持有看涨期权; dirc=-1(空仓(short position)) 即持有看跌期权
        ...
        VSI_commission = (atm_num+max_otm_num)*option_commission # 平台交易的佣金费用
        
    return total_margin, VSI_opencost, VSI_commission # 总的保证金， 期权头寸(position)的历史持仓价值, 期权交易的佣金费用
...
def get_margin_value_by_close(last_str, ETF_close, ETF_pre_close, option_current_date, option_oneday, contract_multiplier, option_commission:int = 3, cash:int = 1000000, risk_expo:float = 0.2): # option_current_date or option_current_month???
    ...
    return total_margin, VSI_call_value, opencost_change, otm_commission, far_cash_delta, last_str # 总的持仓保证金 当前持仓的市场价值 Delta调整导致的开仓成本变化
    #  期权交易产生的佣金 调整后持仓的Delta暴露 更新后的持仓状态字符串
...
def get_positions(factor_df, option_iv, option_current_month, option_commission:int = 3, otm_level:str = 'otm1_call', open_minTime:int = 4, risk_expo:float = 0.2, cash:int = 1000000):
    ...
    for i in range(1,factor_df.shape[0]):
        ...
        if (factor_df.loc[i, 'final_signal'] == -1)&(factor_df.loc[i,'final_direction'] == -1):  # 开空仓
            ...
            VSI_call_opencost = VSI_call_value + opencost_change
            VSI_call_commission = option_commission*(max_otm_num+atm_num) + otm_commission
        ...
        #  close_short VSI = long otm + short atm  VSI回到50分位数附近,且组合为净空头头寸
            ...
            if (factor_df.loc[i, 'final_signal'] == 2): # 平空仓 + 开多仓
                ...
                VSI_call_opencost = VSI_call_value + close_short_VSI_opencost + opencost_change                  # 新开仓成本+平空仓成本（移仓换月成本）
                VSI_call_commission = option_commission*(max_otm_num+atm_num) + close_short_VSI_commission + otm_commission # 新开仓成本+平空仓手续费（移仓换月手续费）
                ...
                # 无论有没有移仓换月，都有close_long_VSI_opencost, close_short_VSI_commission, 不同情况数值不同
                VSI_call_opencost= close_short_VSI_opencost + opencost_change  # 期权历史持仓价值
                VSI_call_commission = close_short_VSI_commission + otm_commission
        ...
        #  close_long VSI = short otm + long atm
            ...
            if (factor_df.loc[i, 'final_signal'] == -2): # 平多仓 + 开空仓
                ...
                # 无论有没有移仓换月，都有close_long_VSI_opencost, close_short_VSI_commission, 不同情况数值不同
                VSI_call_opencost = VSI_call_value + close_long_VSI_opencost + opencost_change
                VSI_call_commission = option_commission*(max_otm_num+atm_num) + close_long_VSI_commission + otm_commission
                ...
                # 无论有没有移仓换月，都有close_long_VSI_opencost, close_short_VSI_commission, 不同情况数值不同
                VSI_call_opencost = close_long_VSI_opencost + opencost_change # 期权历史持仓价值
                VSI_call_commission = close_long_VSI_commission + otm_commission
        ...
        if (factor_df.loc[i,'final_signal'] == 0)&(factor_df.loc[i,'final_direction'] == 0):
            total_margin = 0
        total_margin_list.append(total_margin)
    ...
    print('22-24最终结果',factor_df.tail())
    factor_df.to_csv(f'22-24_{otm_level[:4]}_atm_pnl_df.csv')
    # print('充值！！！', factor_df[factor_df['add_cash']!=0])
    # print('不合格的位置',  factor_df.loc[abs(factor_df['close_cash_delta'])>200000,['date','final_signal','final_direction','far_cash_delta','close_cash_delta','accum_call_pnl']])
    return factor_df
...
def get_max_drawdown(factor_df:pd.DataFrame, init_cash = 1000000):
    ...
    factor_df.to_csv('22-24_otm1_atm_drawdown_return.csv')
...
def get_VSI_nocosts_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.legend()
    plt.savefig(f'22-24_{otm_level[:4]}_atm_pnls_nocost.png')
    plt.show()                      
...
def get_VSI_costs_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.legend()
    plt.savefig(f'22-24_{otm_level[:4]}_atm_pnls_costs.png')
...
def get_cash_delta_plot(plot_df, otm_level:str = 'otm1_call'):
    ...
    plt.show()
    plt.savefig(f'22-24_{otm_level[:4]}_atm_cash_delta_dist.png')
...
def get_margin_plot(plot_df, otm_level:str = 'otm1_call'):
    ...
    plt.show()
    plt.savefig(f'22-24_{otm_level[:4]}_atm_total_margin.png')
...
if __name__ =="__main__":
    ...
    ETF_daily = get_strikes(ETF_daily, start_date = '2022-02-11', end_date = '2024-01-10') 
