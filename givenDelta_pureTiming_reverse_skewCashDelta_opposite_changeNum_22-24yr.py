...
def get_strikes(ETF_daily:pd.DataFrame, start_date:str = '2022-02-11', end_date:str = '2024-01-10'): 
    ...
    ETF_daily.to_csv('changeNum_22-24_ETF_daily.csv')
    return ETF_daily
...
def get_timing_signal(option_iv, otm_level = 'otm1_call', timing_path = '/home/zhangyan/option_skew/timing_signal.csv', start_date='2022-01-28', end_date = '2024-01-10'):
    ...
    factor_df.to_csv(f'changeNum_22-24_{otm_level[:4]}_atm_timing_test.csv')
    return factor_df
...
def get_open_infos(far_atm_delta, far_max_otm_delta, direction, ETF_open, ETF_pre_close, new_atm_open, new_max_otm_open, contract_multiplier, new_atm_delta, new_max_otm_delta, init_cash = 1000000, risk_expo = 0.2):
    otm_num = int(init_cash/(far_max_otm_delta*ETF_open*contract_multiplier))
    atm_num = int(init_cash*(1+risk_expo)/(far_atm_delta*ETF_open*contract_multiplier))
...
def get_margin_value_by_close(last_str, ETF_close, ETF_pre_close, option_current_date, option_oneday, contract_multiplier, option_commission:int = 3, cash:int = 1000000, risk_expo:float = 0.2): # option_current_date or option_current_month???
    ...
    while abs(far_cash_delta) > cash*risk_expo:
        ...
        far_cash_delta = contract_multiplier*dirc*(max_otm_num*max_otm_delta - atm_num*atm_delta)*ETF_close
        # if date == '2023-08-14':
        #     print(max_otm_num, far_cash_delta)
...
def get_positions(factor_df, option_iv, option_current_month, option_commission:int = 3, otm_level:str = 'otm1_call', open_minTime:int = 4, risk_expo:float = 0.2, cash:int = 1000000):
    ...
    print('changeNum_22-24最终结果',factor_df.tail())
    factor_df.to_csv(f'changeNum_22-24_{otm_level[:4]}_atm_pnl_df.csv')
...
def get_max_drawdown(factor_df:pd.DataFrame, init_cash = 1000000):
    factor_df = factor_df[(factor_df['accum_call_pnl'] != 0)&(factor_df['accum_call_pnl'].notna())].copy()
    # 计算年化收益率水平
    ...
    factor_df.to_csv('changeNum_22-24_otm1_atm_drawdown_return.csv')
...
def get_VSI_nocosts_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.legend()
    plt.savefig(f'changeNum_22-24_{otm_level[:4]}_atm_pnls_nocost.png')
    plt.show()                      
...
def get_VSI_costs_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.legend()
    plt.savefig(f'changeNum_22-24_{otm_level[:4]}_atm_pnls_costs.png')
...
def get_cash_delta_plot(plot_df, otm_level:str = 'otm1_call'):
    ...
    plt.show()
    plt.savefig(f'changeNum_22-24_{otm_level[:4]}_atm_cash_delta_dist.png')
...
def get_margin_plot(plot_df, otm_level:str = 'otm1_call'):
    ...
    plt.show()
    plt.savefig(f'changeNum_22-24_{otm_level[:4]}_atm_total_margin.png')
