...
def get_timing_signal(option_iv, otm_level = 'otm1_call', timing_path = '/home/zhangyan/option_skew/timing_signal.csv', start_date='2023-01-01', end_date = '2023-12-29'):
    ...
    factor_df.to_csv(f'pureTime_v2_{otm_level[:4]}_atm_timing_test.csv')
    return factor_df
...
def get_positions(factor_df, option_iv, option_current_month, option_commission:int = 3, otm_level:str = 'otm1_call', open_minTime = 4, risk_expo = 0.2):
    ...
    for i in range(1,factor_df.shape[0]):
        ...
        if (factor_df.loc[i, 'final_signal'] == -1)&(factor_df.loc[i,'direction'] == -1):  # 开空仓
            ...
            factor_df.loc[i, 'VSI_call_opencost'] = VSI_call_value
            factor_df.loc[i, 'VSI_call_commission'] = option_commission*(max_otm_num+atm_num)
        ...
        #  close_short VSI = long otm + short atm  VSI回到50分位数附近,且组合为净空头头寸
            ...
            if (factor_df.loc[i, 'final_signal'] == 2): # 平空仓 + 开多仓
                ...
                factor_df.loc[i, 'VSI_call_opencost'] = VSI_call_value + close_short_VSI_opencost                                   # 新开仓成本+平空仓成本（移仓换月成本）
                factor_df.loc[i, 'VSI_call_commission'] = option_commission*(max_otm_num+atm_num) + close_short_VSI_commission  # 新开仓成本+平空仓手续费（移仓换月手续费）
                ...
                # 无论有没有移仓换月，都有close_long_VSI_opencost, close_short_VSI_commission, 不同情况数值不同
                factor_df.loc[i, 'VSI_call_opencost'] = close_short_VSI_opencost # 期权历史持仓价值
                factor_df.loc[i, 'VSI_call_commission'] = close_short_VSI_commission
        ...
        #  close_long VSI = short otm + long atm
            ...
            if (factor_df.loc[i, 'final_signal'] == -2): # 平多仓 + 开空仓
                ...
                # 无论有没有移仓换月，都有close_long_VSI_opencost, close_short_VSI_commission, 不同情况数值不同
                factor_df.loc[i, 'VSI_call_opencost'] = VSI_call_value + close_long_VSI_opencost
                factor_df.loc[i, 'VSI_call_commission'] = option_commission*(max_otm_num+atm_num) + close_long_VSI_commission
                # if (maturity_date!=last_maturity_date):
                ...
                # 无论有没有移仓换月，都有close_long_VSI_opencost, close_short_VSI_commission, 不同情况数值不同
                factor_df.loc[i, 'VSI_call_opencost'] = close_long_VSI_opencost # 期权历史持仓价值
                factor_df.loc[i, 'VSI_call_commission'] = close_long_VSI_commission
            ...
            if (last_max_otm_inst != max_otm_inst)|(last_atm_inst != atm_inst):  
                ...
                factor_df.loc[i, 'VSI_call_opencost'] = VSI_opencost # 期权历史持仓价值
                factor_df.loc[i, 'VSI_call_commission'] = VSI_commission
        ...
        # T0 收盘后 结算保证金 进行风控
        strike_deltaRate_posRate_list.append(strike_deltaRate_posRate)
        last_str = strike_deltaRate_posRate_list[-1]  # 最新的strike_deltaRate_posRate
        if last_str is not None:
    ...
    print('pureTime最终结果',factor_df.tail())
    factor_df.to_csv(f'pureTime_v2_{otm_level[:4]}_atm_pnl_df.csv')
    return factor_df
...
def get_max_drawdown(factor_df:pd.DataFrame, init_cash = 1000000):
    ...
    factor_df.to_csv('pureTime_v2_otm1_atm_drawdown_return.csv')
...
def get_VSI_nocosts_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.legend()
    plt.savefig(f'pureTime_v2_{otm_level[:4]}_atm_pnls_nocost.png')
    plt.show()                      
...
def get_VSI_costs_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.legend()
    plt.savefig(f'pureTime_v2_{otm_level[:4]}_atm_pnls_costs.png')
...
def get_cash_delta_plot(plot_df, otm_level:str = 'otm1_call'):
    ...
    plt.xlabel('cash_delta')
    plt.ylabel('days', color= 'tab:blue')
     
    plt.title('Cash Delta Distribution')  
    plt.show()
    plt.savefig(f'pureTime_v2_{otm_level[:4]}_atm_cash_delta_dist.png')
...
if __name__ =="__main__":
    ...
    option_current_month_iv = get_parallel_option_current_month(ETF_daily, trading_date_list, min_ttm = 5, open_minTime = 4)  # min_ttm = 5  # 得到 option iv & greeks : call_min  信号上移仓换月的处理（>=10）
