...
# 返回标的 min df
def get_strikes(ETF_daily:pd.DataFrame): 
    ''' 300ETF 期权价格变动单位
    price_gaps = {
        3:0.05,
        5:0.1,
    ...
    for i in ETF_daily.index:
        ...
        if (ETF_daily.loc[i,'ETF_open']*100%10) < 5: # 虚二档 平值
        ...
        ETF_daily.loc[i,'otm1_call'] = round(ETF_daily.loc[i,'atm_call']+gap,1)
        ETF_daily.loc[i,'otm2_call'] = round(ETF_daily.loc[i,'atm_call']+2*gap,1)
        ETF_daily.loc[i,'otm3_call'] = round(ETF_daily.loc[i,'atm_call']+3*gap,1)
        ETF_daily.loc[i,'otm4_call'] = round(ETF_daily.loc[i,'atm_call']+4*gap,1)
        ETF_daily.loc[i,'itm1_call'] = round(ETF_daily.loc[i,'atm_call']-gap,1)
        ETF_daily.loc[i,'itm2_call'] = round(ETF_daily.loc[i,'atm_call']-2*gap,1)
        ETF_daily.loc[i,'itm3_call'] = round(ETF_daily.loc[i,'atm_call']-3*gap,1)
        ETF_daily.loc[i,'itm4_call'] = round(ETF_daily.loc[i,'atm_call']-4*gap,1)
    ETF_daily.to_csv('v0_ETF_daily.csv')
    return ETF_daily
...
def get_option_daily(date, ETF_daily, signal_ttm:int = 57, open_minTime:int = 4, close_minTime:int = 229):
    path = f'/home/zhangyan/option_skew/data-20231229/{date}/{date}_510300.XSHG_call_info_df.csv'
    option_traded = pd.read_csv(path, index_col=0)
    
    option_dict_list = [] 
    for column in ['atm_call','otm1_call','otm2_call','otm3_call','otm4_call','itm1_call','itm2_call','itm3_call','itm4_call']: 
        ...
        sub_path = f'/home/zhangyan/option_skew/data-20231229/{date}/{date}_{option_code}_{symbol}_greeksSeries.csv'
...
def get_parallel_option_daily(ETF_daily, trading_date_list:list, signal_ttm:int = 45, open_minTime:int = 4, close_minTime:int = 229):
    ...
    # option_daily_df.to_csv('23_45_option_daily.csv')
    return option_daily_df
...
def get_timing_signal(option_iv, otm_level = 'otm1_call', timing_path = '/home/zhangyan/option_skew/timing_signal.csv', start_date='2023-01-01', end_date = '2023-12-29'):
    ...
    factor_df.to_csv(f'pureTime_adj_{otm_level[:4]}_atm_timing_test.csv')
    return factor_df
...
def get_margin_back(date, ETF_pre_close, last_str, option_current_date, last_option_current_date, contract_multiplier, last_contract_multiplier, option_commission:int = 3, open_minTime:int = 4):
    if last_str is not None:  
        ...
        if (last_max_otm_inst != max_otm_inst)|(last_atm_inst != atm_inst):      
            ...
            last_max_otm_sub_path = f'/home/zhangyan/option_skew/data-20231229/{date}/{date}_{last_max_otm_inst}_{last_max_otm_symbol}_greeksSeries.csv'
            last_atm_sub_path = f'/home/zhangyan/option_skew/data-20231229/{date}/{date}_{last_atm_inst}_{last_atm_symbol}_greeksSeries.csv'
...
def get_margin_value_by_close(last_str, ETF_close, ETF_pre_close, option_current_date, option_oneday, contract_multiplier, option_commission:int = 3, cash:int = 1000000, risk_expo:float = 0.2): # option_current_date or option_current_month???
    ...
    date = option_current_date['time'].values[0]
    max_otm_close = float(option_current_date.loc[option_current_date['strike'] == (max_otm_call), 'close'].values[0])      # 当月合约的价格
    atm_close = float(option_current_date.loc[option_current_date['strike'] == (atm_call), 'close'].values[0])              # 当月合约的价格
    max_otm_delta = float(option_oneday.loc[option_oneday['strike'] == (max_otm_call), 'close_delta'].values[0])      # 远月合约的价格 # option_oneday
    atm_delta = float(option_oneday.loc[option_oneday['strike'] == (atm_call), 'close_delta'].values[0])              # 远月合约的价格
    ...
    # 计算收盘delta暴露 
    old_max_otm_num = max_otm_num
    far_cash_delta = contract_multiplier*dirc*(max_otm_num*max_otm_delta - atm_num*atm_delta)*ETF_close

    while abs(far_cash_delta) > cash*risk_expo:
        if ((dirc == 1)&(far_cash_delta<0))|((dirc == -1)&(far_cash_delta>0)):
            max_otm_num += 1
        else:
            max_otm_num -= 1
        far_cash_delta = contract_multiplier*dirc*(max_otm_num*max_otm_delta - atm_num*atm_delta)*ETF_close
    ...
    max_otm_num_change = max_otm_num - old_max_otm_num
    opencost_change = contract_multiplier*dirc*max_otm_num_change*max_otm_close
    otm_commission = abs(max_otm_num_change)*option_commission

    last_str = f'{max_otm_call}_{atm_call}_{max_otm_num}_{atm_num}_{dirc}'
    return total_margin, VSI_call_value, opencost_change, otm_commission, far_cash_delta, last_str




# 根据 rollover status 简化判断
def get_positions(factor_df, option_iv, option_current_month, option_commission:int = 3, otm_level:str = 'otm1_call', open_minTime = 4, risk_expo = 0.2):
    factor_df.reset_index(drop=True, inplace=True)
    ...
    factor_df['far_cash_delta'] = 0
    factor_df['new_cash_delta'] = 0
    ...
    close_cash_delta_list = []
    close_cash_delta = 0
    close_cash_delta_list.append(close_cash_delta)

    total_margin_list = []
    total_margin = 0
    total_margin_list.append(total_margin)
    total_margin_at_close_list = []
    total_margin_at_close = 0
    total_margin_at_close_list.append(total_margin_at_close)
    ...
    for i in range(1,factor_df.shape[0]):
        ...
        if (factor_df.loc[i, 'final_signal'] == -1)&(factor_df.loc[i,'direction'] == -1):  # 开空仓
            direction = -1
            # print('开空仓', date)
            ...
            factor_df.loc[i,'far_cash_delta'] = far_cash_delta
        ...
        #  close_short VSI = long otm + short atm  VSI回到50分位数附近,且组合为净空头头寸
        elif ((factor_df.loc[i, 'final_signal'] == 2)&(factor_df.loc[i,'direction'] == 1))|((factor_df.loc[i, 'final_signal'] == 1)&(factor_df.loc[i,'direction'] == 0)):   # 平空仓 或有开多仓
            # 无论是否低于25分位数,都要先平空仓
            direction = 1
            total_margin, close_short_VSI_opencost, close_short_VSI_commission = get_margin_back(date, ETF_pre_close, last_str, option_current_date, last_option_current_date, contract_multiplier, last_contract_multiplier, option_commission, open_minTime = open_minTime)
            ...
            if (factor_df.loc[i, 'final_signal'] == 2): # 平空仓 + 开多仓
                ...
                # if (maturity_date!=last_maturity_date):
                #     print('移仓换月了!  date: ', date, 'open opencost', VSI_call_value, 'close_opencost', close_short_VSI_opencost ) 
                # print(date, 'opencost',VSI_call_value ,'CLOSEcost', close_short_VSI_opencost, 'new_max_otm_call',new_max_otm_call,'new_max_otm_pos',new_max_otm_pos,'new_max_otm_open', new_max_otm_open, 'new_atm_pos', new_atm_pos,\
                #     'new_atm_call', new_atm_call,'new_atm_open',new_atm_open, 'contract_multiplier', contract_multiplier,'strike_deltaRate_posRate', strike_deltaRate_posRate)
        ...
        elif  (factor_df.loc[i, 'final_signal'] == 1)&(factor_df.loc[i,'direction'] == 1):   # 开多仓
            direction = 1
            # print('开多仓', date)
            max_otm_num, atm_num, total_margin, new_cash_delta, far_cash_delta = get_open_infos(far_atm_delta, far_max_otm_delta, direction, ETF_open, ETF_pre_close, new_atm_open, new_max_otm_open, contract_multiplier, new_atm_delta, new_max_otm_delta, init_cash = 1000000, risk_expo = risk_expo)
            open_margin = total_margin
        ...
        #  close_long VSI = short otm + long atm
        elif ((factor_df.loc[i, 'final_signal'] == -1)&(factor_df.loc[i,'direction'] == 0))|((factor_df.loc[i, 'final_signal'] == -2)&(factor_df.loc[i,'direction'] == -1)):    # 平多仓 或有开空仓
            # 无论是否高于75分位数,都要先平多仓
            direction = -1
            total_margin, close_long_VSI_opencost, close_long_VSI_commission = get_margin_back(date, ETF_pre_close, last_str, option_current_date, last_option_current_date, contract_multiplier, last_contract_multiplier, option_commission, open_minTime=open_minTime)
            ...
            if (factor_df.loc[i, 'final_signal'] == -2): # 平多仓 + 开空仓
                ...
                # if (maturity_date!=last_maturity_date):
                #     print('移仓换月了!  date: ', date, 'open opencost', VSI_call_value, 'close_opencost', close_long_VSI_opencost ) 
                # print(date, 'opencost',VSI_call_value ,'CLOSEcost', close_long_VSI_opencost, 'new_max_otm_call',new_max_otm_call,'new_max_otm_pos',new_max_otm_pos,'new_max_otm_open', new_max_otm_open, 'new_atm_pos', new_atm_pos,\
                #         'new_atm_call', new_atm_call,'new_atm_open',new_atm_open, 'contract_multiplier', contract_multiplier,'strike_deltaRate_posRate', strike_deltaRate_posRate)
                


    
            elif (factor_df.loc[i,'direction'] == 0):  # 平仓的处理            
                strike_deltaRate_posRate = None
                open_margin = 0
                far_cash_delta = 0
                new_cash_delta = 0
        ...
        elif (factor_df.loc[i, 'final_signal'] == 0)&(factor_df.loc[i,'direction'] != 0): # 持仓信号不变的时刻
            max_otm_call, atm_call, max_otm_num, atm_num, dirc = last_str.split('_')  # 用之前开仓的 strikes, option num 
            max_otm_call = float(max_otm_call)
            atm_call = float(atm_call)
            max_otm_num = float(max_otm_num)
            ...
            if (last_max_otm_inst != max_otm_inst)|(last_atm_inst != atm_inst):  
                ...
                last_atm_sub_path = f'/home/zhangyan/option_skew/data-20231229/{date}/{date}_{last_atm_inst}_{last_atm_symbol}_greeksSeries.csv'
                last_inst_max_otm_df = pd.read_csv(last_max_otm_sub_path, index_col=0)
                last_inst_max_otm_open = last_inst_max_otm_df.loc[4,'option_price']
                last_inst_atm_df = pd.read_csv(last_atm_sub_path, index_col=0)
                last_inst_atm_open = last_inst_atm_df.loc[4,'option_price']

                max_otm_open = option_current_date.loc[option_current_date['strike'] == float(max_otm_call), 'open'].values[0]               # 新合约当日对应的价格
                atm_open = option_current_date.loc[option_current_date['strike'] == float(atm_call), 'open'].values[0]                       # 新合约当日对应的价格
                # print('移仓换月', date, 'last_atm_inst',last_atm_inst,'last_inst_atm_open',last_inst_atm_open,'last_max_otm_inst',last_max_otm_inst,'last_inst_max_otm_open',last_inst_max_otm_open )
                ...
                if close_margin < 0:
                    ...
                    print(' Error! 信号不变的时刻 string', date, last_str, factor_df.loc[i, 'final_signal'], factor_df.loc[i,'direction'])  
                    break
                ...
                VSI_call_opencost = VSI_opencost + opencost_change # 期权历史持仓价值
                VSI_call_commission = VSI_commission + otm_commission

            else: # 没有进行移仓换月
                VSI_call_opencost = opencost_change # 期权历史持仓价值
                VSI_call_commission = otm_commission
            


            # 当前只有open计算的greeks,且greeks记录的都是用open 开仓的strike可能与当前max otm ,atm不同，所以不能直接用new_greeks
            # 无论是否移仓换月，都要计算greeks
            max_otm_delta = option_current_date.loc[option_current_date['strike'] == float(max_otm_call), 'open_delta'].values[0]
            atm_delta = option_current_date.loc[option_current_date['strike'] == float(atm_call), 'open_delta'].values[0]
            far_otm_Delta = option_oneday.loc[option_oneday['strike'] == float(max_otm_call), 'open_delta'].values[0]          
            far_atm_Delta = option_oneday.loc[option_oneday['strike'] == float(atm_call), 'open_delta'].values[0]  

            new_cash_delta = dirc*(max_otm_delta*max_otm_num - atm_delta*atm_num)*ETF_open*contract_multiplier
            far_cash_delta = dirc*(far_otm_Delta*max_otm_num - far_atm_Delta*atm_num)*ETF_open*contract_multiplier
        ...
        if last_str is not None:
            total_margin_at_close, VSI_close_value, opencost_change, otm_commission, close_cash_delta, last_str_update = get_margin_value_by_close(last_str, ETF_close, ETF_pre_close, option_current_date, option_oneday,\
                                                                                                        contract_multiplier, option_commission = option_commission, cash = 1000000, risk_expo = risk_expo)

            strike_deltaRate_posRate = last_str_update 
        ...
        factor_df.loc[i,'VSI_call_opencost'] = VSI_call_opencost + opencost_change
        factor_df.loc[i,'VSI_call_commission'] = VSI_call_commission + otm_commission
        opencost_change = 0
        otm_commission = 0
        VSI_call_opencost = 0
        ...
        if (factor_df.loc[i,'final_signal'] == 0)&(factor_df.loc[i,'direction'] == 0):
            total_margin = 0
        total_margin_list.append(total_margin)
    ...
    factor_df['total_margin_close'] = total_margin_at_close_list
    factor_df['close_cash_delta'] = close_cash_delta_list
    factor_df['far_cash_delta'] = far_cash_delta_list
    factor_df['add_cash'] = add_cash_list
    ...
    print('pureTime 控制cash delta最终结果',factor_df.tail())
    # print('充值！！！', factor_df[factor_df['add_cash']!=0])
    factor_df.to_csv(f'pureTime_adj_{otm_level[:4]}_atm_pnl_df.csv')
    return factor_df
...
def get_max_drawdown(factor_df:pd.DataFrame, init_cash = 1000000):
    ...
    factor_df.to_csv('pureTime_adj_otm1_atm_drawdown_return.csv')
...
def get_VSI_nocosts_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.legend()
    plt.savefig(f'pureTime_adj_{otm_level[:4]}_atm_pnls_nocost.png')
    plt.show()                      
...
def get_VSI_costs_pnl_plot(plot_df, otm_level:str = 'otm1_call'):  
    ...
    plt.subplot(1,2,1)
    # plt.plot(plot_df.index, plot_df['accum_pnl'], color='red', label = 'accumulated total pnl')
    plt.plot(plot_df.index, plot_df['accum_call_pnl'], color='red', label = 'accumulated call pnl')
    # plt.plot(plot_df.index, plot_df['accum_etf_pnl'],color='orange', label = 'accumulated ETF pnl')
    plt.xticks(rotation=45)
    plt.xlabel('days')
    plt.ylabel('pnls')
    plt.title('accumulated pnls')
    ...
    plt.legend()
    plt.savefig(f'pureTime_adj_{otm_level[:4]}_atm_pnls_costs.png')
...
def get_cash_delta_plot(plot_df, otm_level:str = 'otm1_call'):
    plot_df = plot_df[~plot_df['close_cash_delta'].isna()].copy()
    plot_df.reset_index(drop=True, inplace=True)
    plt.figure(figsize = (10,6))
                    
    plt.plot(plot_df['close_cash_delta'])
    plt.xlabel('days')
    plt.ylabel('cash_delta', color= 'tab:blue')
     
    plt.title('Cash Delta Distribution')  
    plt.show()
    plt.savefig(f'pureTime_adj_{otm_level[:4]}_atm_cash_delta_dist.png')


def get_margin_plot(plot_df, otm_level:str = 'otm1_call'):
    plot_df = plot_df[~plot_df['total_margin'].isna()].copy()
    plot_df.reset_index(drop=True, inplace=True)
    plt.figure(figsize = (30,8))
                
    plt.subplot(1,2,1)
    plt.plot(plot_df['total_margin'])
    plt.xlabel('days')
    plt.ylabel('margin', color= 'tab:blue')
    plt.title('margin')

    plt.subplot(1,2,2)
    plt.plot(plot_df['total_margin_close'])
    plt.xlabel('days')
    plt.ylabel('total_margin_at_close', color= 'tab:blue')
    plt.title('total_margin_at_close')
     
    plt.show()
    plt.savefig(f'pureTime_adj_{otm_level[:4]}_atm_total_margin.png')



if __name__ =="__main__":
    target_folder = '/home/zhangyan/option_skew/data-20231229'
    trading_date_list = get_rollingDates(target_folder)
    
    ETF_daily = get_parallel_ETF_Data(trading_date_list)
    ETF_daily = get_strikes(ETF_daily) 
    ...
    factor_df = get_timing_signal(option_current_month_iv, otm_level = 'otm1_call', timing_path = '/home/zhangyan/option_skew/timing_signal.csv')  # 计算距离到期日10天时，移仓换月的情况
    ...
    get_margin_plot(skews_target_df, otm_level = 'otm1_call')
